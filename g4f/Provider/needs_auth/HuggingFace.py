...
class HuggingFace(AsyncGeneratorProvider, ProviderModelMixin):
    ...
    def get_models(cls) -> list[str]:
        if not cls.models:
            ...
            cls.models = [model["id"] for model in requests.get(url, timeout=60).json()]
        ...
        if not cls.image_models:
            ...
            cls.image_models = [model["id"] for model in requests.get(url, timeout=60).json() if model["trendingScore"] >= 20]
